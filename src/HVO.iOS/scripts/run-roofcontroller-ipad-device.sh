...
# Domain-local wrapper: deploys/launches the iPad app on a physical device using the shared MAUI iOS runner
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help) echo "Use: run-roofcontroller-ipad-device.sh [--configuration] [--udid] [--console|--no-console]"; exit 0 ;;
    *) shift ;;
  esac
done
