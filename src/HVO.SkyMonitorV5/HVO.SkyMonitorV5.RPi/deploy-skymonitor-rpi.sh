...
# Project-local deploy script for SkyMonitor V5 (RPi)
...
if [[ -z "${SKYMONITOR_RUNTIME}" ]]; then
  case "${HOST_ARCH}" in
    aarch64) SKYMONITOR_RUNTIME="linux-arm64" ;;
    armv7l|armhf) SKYMONITOR_RUNTIME="linux-arm" ;;
    x86_64) SKYMONITOR_RUNTIME="linux-x64" ;;
    *) echo "Unsupported Docker host architecture '${HOST_ARCH}'. Set SKYMONITOR_RUNTIME explicitly." >&2; exit 1 ;;
  esac
fi
...
docker --context "${DOCKER_CONTEXT}" run --rm \
  ...
  alpine:3.20 \
  sh -c "set -e; mkdir -p /datastore/configuration /datastore/telemetry /datastore/catalogs /datastore/logs /datastore/dataprotection && chmod -R 775 /datastore"

echo "\n→ Ensuring export root directories exist on remote host (${EXPORT_ROOT})..."
docker --context "${DOCKER_CONTEXT}" run --rm \
  -v "${EXPORT_ROOT}":/exports \
  alpine:3.20 \
  sh -c "set -e; mkdir -p /exports/raw /exports/processed && chmod -R 775 /exports"
