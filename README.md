...
<!-- Coverage badge - uncomment after Gist setup (see docs/guides/coverage-badge-setup.md) -->
...
> **📊 Coverage Badge Setup**: The workflow is configured to generate coverage badges. To enable the badge display:
> 1. Create a GitHub Gist with filename `coverage-badge.json` ([create here](https://gist.github.com))
> 2. Set repository variable `COVERAGE_GIST_ID` to your Gist ID (Settings → Secrets and variables → Actions → Variables)
> 3. Create a Personal Access Token with `gist` scope ([create here](https://github.com/settings/tokens))
> 4. Add it as repository secret `GIST_TOKEN` (Settings → Secrets and variables → Actions → Secrets)
> 5. Uncomment the badge line above and replace `YOUR_GIST_ID_HERE` with your Gist ID
>
> **Coverage Collection**: Enabled via `src/coverage.runsettings` using Cobertura format. Coverage artifacts are generated by test jobs and processed automatically on main branch pushes. See [Coverage Badge Setup Guide](docs/guides/coverage-badge-setup.md) for detailed instructions.
