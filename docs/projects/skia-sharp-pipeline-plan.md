...
- [ ] DEFERRED TO FUTURE RELEASE. Parallel filter execution would require substantial refactoring (per-filter surfaces, thread-safe caches, deterministic compositing) for an expected ~10 ms best-case gain on current hardware. Keeping this parked until latency targets change or higher-capacity hardware is adopted.
