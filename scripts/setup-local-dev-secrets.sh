...
show_setup_instructions() {
  ...
  log ""
  log "The dev container now uses remoteEnv to pass environment variables from your Mac."
  log ""
  log "IMPORTANT: You need to set these environment variables on your Mac BEFORE launching VS Code:"
  log ""
  log "Option 1 - Add to your shell profile (~/.zshrc or ~/.bash_profile):"
  log "  export HVO_SECRET__SSH__PRIVATE_KEY_B64='your_base64_private_key'"
  log "  export HVO_SECRET__SSH__PUBLIC_KEY_B64='your_base64_public_key'"
  log "  export HVO_SECRET__WEBSITEV9__DB_CONNECTION='your_db_connection'"
  log "  # ... other secrets as needed"
  log ""
  log "Option 2 - Set them in your current shell before launching VS Code:"
  log "  export HVO_SECRET__SSH__PRIVATE_KEY_B64='your_base64_private_key'"
  log "  code /path/to/HVOv9"
  log ""
  log "Option 3 - Use a .env file loader in your shell profile"
  log ""
  log "After setting the environment variables:"
  log "1. Restart your terminal or run 'source ~/.zshrc'"
  log "2. Launch VS Code from that terminal: 'code /path/to/HVOv9'"
  log "3. Rebuild the dev container to pick up the environment variables"
  log ""
  log "To verify variables are set: 'printenv | grep HVO_SECRET'"
  log ""
